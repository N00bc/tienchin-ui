<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="channelList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="渠道类型" prop="type" :show-overflow-tooltip="true" width="150">
        <!-- 插槽最新写法 -->
        <template #default="scope">
          <template v-for="(type,index) in channel_type">
            <el-tag :key="index" v-if="type.value == scope.row.type" :type="type.elTagType">
              {{ type.label }}
            </el-tag>
          </template>
        </template>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-tooltip content="修改" placement="top" v-if="scope.row.channelId !== 1">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                       v-hasPermi="['tienchin:channel:edit']"></el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top" v-if="scope.row.channelId !== 1">
...
import {addChannel, deleteChannel, getChannel, listChannel, updateChannel} from "@/api/tienchin/channel";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除渠道编号为"' + channelIds + '"的数据项?').then(function () {
    return deleteChannel(channelIds);
  }).then(() => {
    getList();
